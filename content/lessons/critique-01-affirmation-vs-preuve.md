---
id: "critique-01"
slug: "critique-01-affirmation-vs-preuve"
school: "CRITIQUE"
level: 1
order: 1
editorialStatus: "APPROVED"
titleFr: "Une affirmation n'est pas une preuve"
titleAr: "الدعوى ليست دليلاً"
summaryFr: "Apprendre à distinguer rigoureusement entre une affirmation, un argument et une preuve probante."
summaryAr: "التفريق الدقيق بين مجرد الدعوى، والاحتجاج، والدليل القاطع المعتبر."
methodologyPrincipleFr: "Toute prétention dépourvue de preuve recevable est nulle et sans effet."
methodologyPrincipleAr: "الدعاوى إذا لم تقم عليها بينات، فأصحابها أدعياء."
historicReference:
  author: "Ibn Qayyim al-Jawziyya"
  work: "Iʿlâm al-Muwaqqiʿîn ʿan Rabb al-ʿÂlamîn"
  editionVolumePage: "Tome 1, p. 75 (Éd. Dâr Ibn al-Jawzî)"
  quoteArOriginal: "والدعوى المجردة عن البرهان لا تفيد شيئا، ولو قبلت الدعاوى بلا برهان لادعى كل أحد ما يشتهيه"
  quoteArVocalized: "وَالدَّعْوَى الْمُجَرَّدَةُ عَنِ الْبُرْهَانِ لَا تُفِيدُ شَيْئًا، وَلَوْ قُبِلَتِ الدَّعَاوَى بِلَا بُرْهَانٍ لَادَّعَى كُلُّ أَحَدٍ مَا يَشْتَهِيهِ"
  quoteArNormalized: "والدعوى المجردة عن البرهان لا تفيد شيئا ولو قبلت الدعاوى بلا برهان لادعى كل احد ما يشتهيه"
  translationFr: "L'affirmation dénuée de preuve probante ne sert à rien. Si les affirmations étaient acceptées sans preuve, chacun revendiquerait tout ce qu'il désire."
  citationStatus: "VERIFIED_VERBATIM"
  verifiedAt: "2026-09-21"
  verifiedBy: "reviewer-01"
authorId: "author-01"
reviewerId: "reviewer-01"
reviewedAt: "2026-09-21"
lastVerifiedAt: "2026-09-21"
quizzes:
  - id: "quiz-c01-1"
    questionFr: "Un orateur s'exclame avec ferveur : 'Cette pratique est formellement interdite par l'ensemble des savants !' Que représente méthodologiquement cette phrase ?"
    questionAr: "قال خطيب بحماس: 'هذا الفعل محرم بإجماع العلماء!' ماذا يمثل هذا الإطلاق منهجياً؟"
    order: 1
    options:
      - textFr: "Une preuve péremptoire car l'orateur est éloquent et convaincu."
        textAr: "دليل قاطع لأن الخطيب بليغ ومتأكد."
        isCorrect: false
        feedbackFr: "L'éloquence ou la certitude psychologique de l'orateur ne transforment jamais une parole en preuve."
        feedbackAr: "الفصاحة واليقين النفسي للمتكلم لا يجعلان كلامه دليلاً شرعياً بحال."
      - textFr: "Une simple affirmation (daʿwâ) qui exige elle-même d'être prouvée par des sources documentées."
        textAr: "مجرد دعوى تحتاج في نفسها إلى إثبات ونقل موثق."
        isCorrect: true
        feedbackFr: "Exact ! Une allégation d'interdiction ou de consensus reste une simple prétention tant que les sources textuelles ne sont pas exhibées."
        feedbackAr: "أحسنت! ادعاء التحريم أو الإجماع مجرد دعوى تفتقر إلى البينة ونقل المصادر."
      - textFr: "Un argument suffisant car personne dans l'assemblée n'a osé le contredire."
        textAr: "حجة كافية لعدم معارضة أحد من الحاضرين له."
        isCorrect: false
        feedbackFr: "Le silence ou l'absence d'opposition immédiate d'un public n'a aucune valeur d'authentification scientifique."
        feedbackAr: "سكوت الجمهور أو عدم اعتراضهم لا ينهض دليلاً على صحة الدعوى."
---

# 1. Mise en situation (Problématique)

Sur les réseaux sociaux, dans un groupe de discussion ou lors d'une intervention orale, nous entendons quotidiennement des formules catégoriques :
> « Ce comportement annule la prière selon l'unanimité ! »  
> « Ce livre contient des égarements majeurs ! »  
> « Cette règle est le pilier central de la Sunnah ! »

Face à une telle affirmation, le réflexe spontané de la majorité des gens est d'évaluer **l'émotion**, **la notoriété** de la personne qui parle ou **la beauté** du propos.

Mais pour celui qui recherche la vérité selon les règles de la rigueur scientifique, une question fondamentale s'impose :
**Ce qui a été énoncé est-il une preuve (*dalîl*), ou simplement une prétention (*daʿwâ*) qui exige elle-même d'être prouvée ?**

---

# 2. Définition méthodologique : Le triptyque du discours

Les savants de l'Islam ont très tôt formalisé la distinction entre trois niveaux :

1. **L'Affirmation (الدعوى — *Ad-Daʿwâ*)** :  
   C'est l'énoncé d'un jugement ou d'un fait. Elle n'a, par elle-même, aucune force probante. Qu'elle soit prononcée par un inconnu ou par une personnalité célèbre, elle demeure une simple prétention tant qu'elle n'est pas étayée.

2. **L'Argument (الاحتجاج — *Al-Iḥtijâj*)** :  
   C'est la tentative de justifier l'affirmation en mobilisant un élément extérieur (un verset, un hadith, une analogie, un consensus). Mais citer un texte ne suffit pas : encore faut-il que ce texte soit authentique et que sa portée s'applique réellement à la question.

3. **La Preuve probante (الدليل — *Ad-Dalîl*)** :  
   C'est l'élément scripturaire ou rationnel recevable qui établit la véracité de l'affirmation de manière indiscutable ou fortement probable selon les règles admises du savoir.

> « L'affirmation dénuée de preuve probante ne sert à rien. Si les affirmations étaient acceptées sans preuve, chacun revendiquerait tout ce qu'il désire. »  
> — *Ibn Qayyim al-Jawziyya (Iʿlâm al-Muwaqqiʿîn ʿan Rabb al-ʿÂlamîn, T.1, p.75)*

---

# 3. Le piège méthodologique fréquent

### ❌ Le piège : Confondre l'intensité de l'affirmation avec sa véracité
Croire que plus une affirmation est répétée, clamée avec assurance, ou formulée avec un vocabulaire intimidant (*« sans aucun doute », « tous les imams sans exception »*), plus elle est vraie.

### ✅ La règle de vérification : *Al-Bayyina ʿalâ al-muddaʿî*
En sciences islamiques comme en droit universel, la règle immuable est :  
> **« C'est au demandeur (celui qui affirme) d'apporter la preuve. »** (*Al-Bayyinatu ʿalâ al-muddaʿî*).

Tant que la personne qui affirme n'a pas produit le texte exact, le recueil, la chaîne ou le document original, son affirmation est considérée comme **scientifiquement non avenue**. Elle n'engage personne et ne peut fonder aucun jugement religieux.

---

# 4. Exercice d'application (Mini-Quiz)

Consultez le quiz interactif ci-dessus pour tester votre réflexe face à une affirmation sans preuve.
